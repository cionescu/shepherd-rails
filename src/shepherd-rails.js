...
const _excluded2 = [ "mainAxis", "crossAxis", "fallbackPlacements", "fallbackStrategy", "flipAlignment" ], _excluded4 = [ "mainAxis", "crossAxis", "limiter" ];
...
const flip = function flip(options) {
  ...
  return {
    ...
    async fn(middlewareArguments) {
      ...
      const {mainAxis: checkMainAxis = true, crossAxis: checkCrossAxis = true, fallbackPlacements: specifiedFallbackPlacements, fallbackStrategy: fallbackStrategy = "bestFit", flipAlignment: flipAlignment = true} = options, detectOverflowOptions = _objectWithoutPropertiesLoose(options, _excluded2);
      const side = getSide(placement);
...
const limitShift = function limitShift(options) {
  ...
  return {
    ...
    fn(middlewareArguments) {
      const {x: x, y: y, placement: placement, rects: rects, middlewareData: middlewareData} = middlewareArguments;
      const {offset: offset = 0, mainAxis: checkMainAxis = true, crossAxis: checkCrossAxis = true} = options;
      const coords = {
        x: x,
        y: y
      };
...
function autoUpdate(reference, floating, update, options) {
  ...
  const {ancestorScroll: _ancestorScroll = true, ancestorResize: ancestorResize = true, elementResize: elementResize = true, animationFrame: animationFrame = false} = options;
...
function floatingUIposition(step, shouldCenter) {
  return _ref => {
    let {x: x, y: y, placement: placement, middlewareData: middlewareData} = _ref;
    if (!step.el) {
      return step;
    }
    if (shouldCenter) {
...
function makeOverlayPath(_ref) {
  let {width: width, height: height, x: x = 0, y: y = 0, r: r = 0} = _ref;
